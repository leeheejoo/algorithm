...
function depthFirstSearch(graph, startVertex, callbacks) {
    ...
    let initiatedCallback = initCallbacks(callbacks);
    callbacks.allowTraversal({ nextVertex:startVertex });
    depthFirstSearchRecursive(graph, startVertex, previousVertex, initiatedCallback);
}
...
function test() {
    ...
    depthFirstSearch(grahp, v1, callback);
}
